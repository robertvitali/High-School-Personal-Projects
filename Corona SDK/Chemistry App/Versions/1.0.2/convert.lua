...
function scene:create (event)
	...
	local notesText = display.newText("Convert", display.contentWidth/2, whiteBar2.y, native.systemFontBold, 50)
		notesText:setFillColor(0,0,0)

	--Text Field 
	local textField1 = native.newTextField(display.contentWidth/2, whiteBar.y + 130, .9*display.contentWidth, 70)
	...
	--Text Field
	local textField2 = native.newTextField(display.contentWidth/2, textField1.y+ 130, .9*display.contentWidth, 70)
		textField2.placeholder = "Enter grams/moles, no units."
		textField2.inputType = "number"
		textField2.size = 46
		textField2:setReturnKey("done")

	local rectangle2 = display.newRoundedRect(textField2.x, textField2.y, textField2.width + 15, textField2.height + 15, 10)

	--Moles button
	local molesButton = display.newText("Convert To Moles", display.contentWidth/2, textField2.y + 100, native.systemFontBold, 38)

	--Grams button
	local gramsButton = display.newText("Convert To Grams", display.contentWidth/2, molesButton.y + 100, native.systemFontBold, 38)

	--Value Text
	local valueText = display.newText("", display.contentWidth/2, gramsButton.y + 130, native.systemFontBold, 38)
	...
	local function convertToMoles(event)
		if (textField1.text ~= "") then
			...
			valueText.text = convertValue.."mol"
		end
	end
