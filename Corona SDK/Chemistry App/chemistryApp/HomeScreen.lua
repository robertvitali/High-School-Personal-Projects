...
function scene:create (event)
	...
	local function textListener( event )
	    ...
	    if ( event.phase == "editing" ) then
			...
			elseif(letterVar == "N")then
				if(string.sub(textField1.text, string.len(textField1.text)-1, string.len(textField1.text)-1) == "H") or (string.sub(textField1.text, string.len(textField1.text)-1, string.len(textField1.text)-1) == "h")then
					button1.text = "NO3"
					button2.text = "NO2"
					button3.text = "NO4"
				else
	    ...
	    elseif(event.phase == "ended" or event.phase == "submitted")then
	    	keyboardListener()
	    	firebaseAnalytics.logEvent("select_content", {content_type = "Text Field Event"})
	    end
	    molarMass.text = ""
		molarMassValue = 0
	    molarMassCalculate()
	...
	local function replaceText1(event)
		...
		firebaseAnalytics.logEvent("select_content", {content_type = "Predictive Button"})
	end
	...
	local function clearText(event)
		...
		molarMass.text = ""
		multiplier = 1
		firebaseAnalytics.logEvent("select_content", {content_type = "Clear Text"})
	end
	...
	local function copyText(event)
		...
		copyFunction()
		keyboardListener()
		firebaseAnalytics.logEvent("select_content", {content_type = "Copy Text"})
	end
