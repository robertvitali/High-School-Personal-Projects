...
function scene:create (event)
	...
	local textSize = display.contentHeight/20
	...
	button1Text = display.newText("Encrypt Message", button1.x, button1.y, native.systemFontBold, textSize)
	...
	button2Text = display.newText("Decrypt Message", button2.x, button2.y, native.systemFontBold, textSize)
	...
	ads.show( "banner", { x=0, y=display.contentHeight, appID } )
...
local function ButtonTap1 (event)
	...
	firebaseAnalytics.logEvent("select_content", {content_type = "Encrypt"})
	--ads.hide()
end

local function ButtonTap2 (event)
	composer.gotoScene("Decrypt")
	firebaseAnalytics.logEvent("select_content", {content_type = "Decrypt"})
	--ads.hide()
end
