...
function scene:create (event)
	...
	local temp = display.newText(UserInfo.dBand, display.contentWidth/2, display.contentHeight-430, native.systemFont, 26)
...
--		temp = display.newText(dBandField.text, display.contentWidth/2, 0, native.systemFont, 20)
--	end
--	temp:setFillColor(0,0,0)
	...
	local notes1 = native.newTextBox(display.contentWidth/2, display.contentHeight/2-10, 310, 225)
		notes1.text = UserInfo.dBandHomework
		notes1.size=28
		notes1:setTextColor(0,0,0)
	
		if(UserInfo.dBandHomework == "")then
	...
	dBandGroup1:insert(background)
	dBandGroup1:insert(temp)
	dBandGroup1:insert(back)
	dBandGroup1:insert(save)
	dBandGroup1:insert(reset)
	dBandGroup1:insert(notesText)
	dBandGroup1:insert(notes1)
...
function keyboardListener(event)
	...
	local function saveButton(event)
		UserInfo.dBandHomework = notes1.text
...
2) THE TITLE TEXT (TEMP) DOESN'T CHANGE WHEN THE dBandFIELD TEXT IS CHANGED
]]