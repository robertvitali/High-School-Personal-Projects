...
function scene:create (event)
	...
	asteoroidTimer2 = timer.performWithDelay (900000, spawnMoreAsteroids, 1)
	...
	starTimer2 = timer.performWithDelay (99900, spawnMoreStars, 1)
	...
	local function scoringandCollision()
	...
	timer.performWithDelay(50, scoringandCollision, 0)




	local function scoreUpdating()
	...
	timerScoreUpdating = timer.performWithDelay(10, scoreUpdating, 0)

	playScreenGroup:insert(background)
	playScreenGroup:insert(bar)
	playScreenGroup:insert(rocketShip)
--	playScreenGroup:insert(asteroid[numAsteroids])
--	playScreenGroup:insert(star[numAsteroids])
...
local function switchScene (event)
	if(health < 1) then
		...
		timer.pause(asteoroidTimer2)
		timer.pause(starTimer1)
		timer.pause(starTimer2)
		timer.pause(timerScoreUpdating)
		playScreenGroup.alpha = 0
		composer.gotoScene("restart", {time = 10})
	end
end
...
function scene:hide(event)
	Runtime:removeEventListener("collision", switchScene)
