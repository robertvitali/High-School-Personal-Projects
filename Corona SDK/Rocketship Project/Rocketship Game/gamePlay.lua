...
function scene:create (event)
	...
	physics.addBody(rocketShip, "kinematic", {density = 1, friction = 0, bounce = .2})
	...
	local function loadAsteroid()
		for i=1,numAsteroids do
			if(health > 0) then
				...
				physics.addBody(asteroid[numAsteroids], {density = 2, bounce = .2, radius = 44})
	...
	local function loadStars()
		for i=1,numStars do
			if(health > 0) then
				...
				physics.addBody(star[numStars], {density = .4, bounce = .2, radius = 13})
	...
	local function cleanup()
	   	for i=1,table.getn(asteroid) do
	     	if(asteroid[i].myName~= nil) then
	         	asteroid[i]:removeSelf()
	        	asteroid[i].myName=nil
			end 
		end
		--[[
		for x=1,table.getn(star) do
			if(star[x].myName~= nil) then
				star[x]:removeSelf()
				star[x].myName=nil
			end
		end
